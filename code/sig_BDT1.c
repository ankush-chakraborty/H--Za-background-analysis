...
void sig_BDT1() {
    TFile *f = TFile::Open("TMVAOutput_1.root");
    ...
    for (Long64_t i = 0; i < tree->GetEntries(); ++i) {
        ...
        bool masscut = (fabs(m_llg - 125) < 2);        
        if (processID == 1) {
        if (masscut)
            h_sig_VBF->Fill(theta);
            c_VBF++;
            if (masscut) sig_VBF++;
        } else if (processID == 2) {
        if (masscut)
            h_sig_ggH->Fill(theta);
            c_ggH++;
            if (masscut) sig_ggH++;
        } else if (processID == 3) {
        if (masscut)
    ...
    */
    ...
    TH1F* h_significance = (TH1F*)h_sig_VBF->Clone("h_significance");
    h_significance->Reset(); // clear contents
    
    for (int i = 1; i <= nbins; ++i) {
        double S = h_sig_VBF->GetBinContent(i);
    ...
    // Optional: Style and draw
    h_significance->SetTitle("Bin-by-bin Significance;#theta [Radians];S/#sqrt{S+B}");
    h_significance->SetLineColor(kOrange);
    h_significance->SetFillColor(kOrange);
