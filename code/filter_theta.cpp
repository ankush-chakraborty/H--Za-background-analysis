...
bool is_dijet_tag(const Event& event) {
    ...
    if (fabs(zg.Rapidity() - 0.5*(j1.rapidity() + j2.rapidity())) > 2.5) return false;
...
int main() {
    ...
    TH1F *h3 = new TH1F("h1", "", nbins, xmin, xmax);
    TH1F *h4 = new TH1F("h2", "", nbins, xmin, xmax);
    ...
    for (const auto& evt : dijet_tag[0]) {
        ...
        if(evt.leptons[0].first > 0) {l1 = evt.leptons[0].second; l2 = evt.leptons[1].second;}
        else {l2 = evt.leptons[0].second; l1 = evt.leptons[1].second;}
        TLorentzVector gamma = evt.photon[0];
